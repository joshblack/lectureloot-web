...
class CheckinsController extends \BaseController {
	...
	public function create($id)
	{
		...
		foreach ($courses as $course) {
			...
			foreach ($meetings as $meeting) {
				...
				if ($meetingPeriod == $nextPeriod->period)
				{
					...
					$error = ['message' => ['error' => ['It\'s too soon to check in']]];
				}
				else if ($meetingPeriod == $currentPeriod->period)
				{
					$checkinMeeting = ($afterStart > $date) ? $meeting : null;
					$error = ['message' => ['error' => ['It\'s too late to check in']]];
				}
				else
				{
					// We can't find a meeting that the user can check into
					$checkinMeeting = null;
					$error = ['message' => ['error' => ['You don\'t have any classes to check into at this time']]];
				}
			}
		}
		...
		{
			array_push($error['message']['error'], 'You can\'t checkin at this time');
		}
		...
		if ($checkinMeeting && ($tolerance > $distanceFromBuilding))
		{
			...
			$success = ['message' => ['success' => 'Success, you have checked in to your class']];
		}
		else
		{ // The user isn't in the right place
			$statusCode = 200;
			$value = 'plain/text';
			array_push($error['message']['error'], 'Location Error, you aren\'t in the right location for the class you are trying to check into.');
		}

		$contents = ($error) ? json_encode($error) : json_encode($success);
		$statusCode = 200;
		$response = Response::make($contents, $statusCode);
		$response->header('Content-Type', $value);
